...
export function createHintAction({ timeStepBefore, positions, isSingle, values = [] }) {
  ...
  return {
    apply: () => {
      ...
      // 为策略管理器在该时间步建立快照（将来自 action 的 positions 保存）
      strategyManager.snapshotLastApplied(nextTs, positions);
    ...
    undo: () => {
      ...
      strategyManager.gotoTimeStep(timeStepBefore);
      strategyGrid.getStrategyGrid().update(($strategyGrid) => {
        $strategyGrid.map((row) => row.map((cell) => {
    ...
    redo: () => {
      const nextTs = timeStepBefore + 1;
      strategyGrid.setTimeStep(nextTs);
      strategyManager.gotoTimeStep(nextTs);
      strategyGrid.getStrategyGrid().update(($strategyGrid) => {
        $strategyGrid.map((row) => row.map((cell) => {
...
export function createBacktrackAction({ fromTimeStep, toTimeStep }) {
  return {
    apply: () => {
      ...
      strategyManager.gotoTimeStep(toTimeStep);
      strategyGrid.getStrategyGrid().update(($strategyGrid) => {
    ...
    undo: () => {
      ...
      strategyManager.gotoTimeStep(fromTimeStep);
      strategyGrid.getStrategyGrid().update(($strategyGrid) => {
