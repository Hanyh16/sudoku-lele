...
class HistoryDGA {
  ...
  _getLatestChild(node) {
    ...
    // 辅助：检查从 fromNode 是否能到达 toNode（用于防止复用导致循环）
  canReach(fromNode, toNode) {
    ...
    while (stack.length > 0) {
      ...
      (node.outgoingEdges || []).forEach(edge => {
        if (edge.to && !visited.has(edge.to)) stack.push(edge.to);
      });
    }
    return false;
  }
  ...
  addNode(state, timestamp, action = null, isTrace = false) {
    ...
    // 检查复用是否会导致循环：如果 existingNode 是 current 的祖先或后代，则不复用
    if (existingNode && (this.canReach(this.current, existingNode) || this.canReach(existingNode, this.current))) {
      try { console.debug('[HistoryDGA.addNode] existing node would create cycle; forcing new node creation', { existingCreatedAt: existingNode.createdAt }); } catch (e) {}
      existingNode = null;
    }
