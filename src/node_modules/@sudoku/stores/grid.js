...
import { CreateCellLinkedList } from './cell';
...
function createUserGrid() {
	...
	return {
		...
		// Allow callers to update the entire underlying writable (delegate)
		update: (updater) => {
			return userGrid.update(updater);
		},

		// Replace the whole user grid with a new snapshot
		setGrid: (newGrid) => {
			return userGrid.set(newGrid);
		},
...
function createStrategyGrid() {
  ...
  return {
    ...
    increaseTimeStep: () => timeStep.update($timeStep => { 
      $timeStep++; 
      return $timeStep;
    }),
    ...
    updateCellCandidates: (positions = null) => {
      strategyGrid.update($strategyGrid => {
        const cellsToUpdate = positions 
          ? positions.map(pos => $strategyGrid[pos.y][pos.x])
          : null;

        for (let row = 0; row < SUDOKU_SIZE; row++) {
          for (let col = 0; col < SUDOKU_SIZE; col++) {
            if (positions && !cellsToUpdate.includes($strategyGrid[row][col])) {
              continue;
            }
            ...
            for (let r = boxStartRow; r < boxStartRow + BOX_SIZE; r++) {
              for (let c = boxStartCol; c < boxStartCol + BOX_SIZE; c++) {
                if ($strategyGrid[r][c].isCellConstant()) {
                  sameBox.push($strategyGrid[r][c].getCurrentCell());
                }
              }
            }
	...
	printCandidates: () => {
		const $grid = get(strategyGrid);
		console.log('=== 候选集 Debug ===');
		for (let row = 0; row < SUDOKU_SIZE; row++) {
		let rowStr = '';
		for (let col = 0; col < SUDOKU_SIZE; col++) {
			const cell = $grid[row][col];
			if (cell.isConstant) {
			rowStr += `[${cell.getCurrentCell()}] `;
			} else {
			rowStr += `{${cell.candidates.join(',') || '-'}} `;
			}
		}
...
// Ensure strategy candidates recompute when user grid changes.
// This keeps strategyGrid.candidates in sync after user entries.
try {
	userGrid.subscribe(() => {
		try { strategyGrid.updateCellCandidates(); } catch (e) { /* ignore */ }
	});
} catch (e) {
	// ignore if subscription can't be set at module init
}