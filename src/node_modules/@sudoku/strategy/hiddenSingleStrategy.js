    ...
    if (type === 'row') {
      for (let r = 0; r < SUDOKU_SIZE; r++) {
        for (let c = 0; c < SUDOKU_SIZE; c++) {
          if (c === col) continue;
          if ((grid[r][c].isCellConstant() && grid[r][c].getCurrentCell() === candidate)
            || (!grid[r][c].isCellConstant() && grid[r][c].candidates.length === 1 
              && grid[r][c].candidates[0] === candidate)) {
      ...
      for (let r = 0; r < SUDOKU_SIZE; r++) {
        ...
        if (!grid[r][col].isCellConstant() && grid[r][col].strategies.length > 0
          && !grid[row][col].relativePos.some(cell => cell.y === r || this.isSameBox(cell, {x: col, y: r}))) {
          for (const strategy of grid[r][col].strategies) {
            if (!grid[row][col].strategies.includes(strategy)) {
              grid[row][col].strategies.push(strategy);
            }
